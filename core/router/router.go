...
import (
	...
	cgrpc "github.com/labulaka521/crocodile/core/grpc"
...
func Run(mode define.RunMode) error {
	var (
	...
	gRPCServer, err = cgrpc.NewgRPCServer(mode)
	if err != nil {
		return err
	}
	...
	default:
		return errors.New("Unsupport mode")
	}
	...
	if mode == define.Server {
		...
		log.Info("Start Run Http Server", zap.String("addr", addr))
	}
